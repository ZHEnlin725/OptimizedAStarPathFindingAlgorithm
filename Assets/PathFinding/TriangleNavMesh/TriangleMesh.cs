...
namespace PathFinding.TriangleNavMesh
{
    ...
    public class TriangleMesh : ITopology<Triangle>
    {
        ...
        {
            return _triangleRoutes.TryGetValue(node, out var routes) ? routes : null;
        }
...
#if UNITY_EDITOR
            Debug.LogError($"Init Triangles Consume {(DateTime.Now - now).TotalMilliseconds}ms ！！！");
#endif
        }
...
#if UNITY_EDITOR
            Debug.LogError($"Init Routes Consume {(DateTime.Now - now).TotalMilliseconds}ms ！！！");
#endif
        }
...
#if UNITY_EDITOR
            Debug.LogError($"Init BSPTree Consume {(DateTime.Now - now).TotalMilliseconds}ms ！！！");
#endif
        }
        ...
        public void DrawTriangle(int index = 0, bool drawNeighbour = false)
        {
            ...
            if (drawNeighbour)
                for (var i = 0; i < _triangleRoutes[_triangles[index]].Count; i++)
                {
